...
/*/public/js/maigoo/v2019/block/brand10/bangdan.js*/
$(document).ready(function(e) {
    var str='榜单说明：'+$('.blockinfo.bangdan .bangdesc .desc').attr('cname')+'十大品牌数据由CN10排排榜技术研究部门和CNPP品牌数据研究部门通过资料收集整理，并基于大数据统计及人为根据市场和参数条件变化的分析研究专业测评而得出，是大数据、云计算、数据统计真实客观呈现的结果，CN10/CNPP是历史悠久且权威公正的测评机构/大数据云计算公司，通过广泛收集整理汇编全球权威数据，结合专业独立调研测评，定期发布更新客观公正的排行榜！目前买购品牌榜手机APP上，已经覆盖了所有行业的知识排行榜、品牌排行榜，购物前先用买购网APP查一查，知识榜、品牌榜，查排行用买购网！原始数据来源于用户企业免费自主申请申报和CN10/CNPP品牌数据部门主动收录研究得出的品牌信息资料库，以及信用指数以及几十项数据统计计算系统生成的行业大数据库，并以企业实力、品牌荣誉、网络投票、网民口碑打分、企业在行业内的排名情况、企业获得的荣誉及奖励情况等为基础，通过特定的计算机模型对广泛的数据资源进行采集分析研究，综合了多家机构媒体和网站排行数据，经人工智能和品牌研究员专业测评，根据市场和参数条件变化计算机程序汇编生成显示在网站上，只有在行业出名、具有规模、影响力、经济实力的企业在才会被系统收录并在网站上面展示出现。<a href="https://www.cnpp.cn/analysis/" target="_blank" class="color1">更多详情>></a>';
	$('.blockinfo.bangdan .bangdesc .desc').html(str);
});
...
/*/public/js/maigoo/v2019/block/publicblock/miaodian.js*/
$(document).ready(function(){
	...
	function focusli(){
		...
		$('#container').find('.blockinfo,.moreall2').each(function(k){
			var _this=$(this);
			if($('#container .miaodian li a[pos='+_this.attr('id').replace('pos_','')+']').length>0){
				if(_this.offset().top+_this.next().height()>=$(window).scrollTop()&&_this.offset().top<$(window).scrollTop()+$(window).height()){
					n++;
					if(n<=1){
