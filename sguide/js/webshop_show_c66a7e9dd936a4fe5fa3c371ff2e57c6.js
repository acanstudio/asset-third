...
/*/public/js/maigoo/v2019/block/webshop/webshopinfo.js*/
$(document).ready(function(e) {
	var catobj=$('.webshopinfo .catlist');
	
	var lineheight=parseInt(catobj.css('line-height'));
	
	var maxnum = 2;

	if(catobj.height()>maxnum*lineheight){
		catobj.addClass('dhidden2');	
		catobj.mouseover(function(){
			catobj.removeClass('dhidden2').addClass('hov');
		}).mouseout(function(){
			catobj.removeClass('hov').addClass('dhidden2');	
		});
	}
...
/*/public/js/maigoo/v2019/block/publicblock/searchselect3.js*/
var thisurl = window.location.pathname+'?dynamic=1';
$(document).ready(function(e) {
	$('.searchselect3').delegate('.selecthead .li','click',function(){
		var i=$(this).index(),selobj=$(this).parents('.searchselect3').find('.licont:eq('+i+')');
		var _this=$(this);
		if(_this.data('keyname')=='pricesort'){//价格
			var pricesort=parseInt(_this.data('val'))==2?1:2;
			window.history.pushState({},0,thisurl+'&'+$(this).data('keyname')+'='+pricesort);
			_this.data($(this).data('keyname'),pricesort);
			_this.data('ajax',1);
			$.ajax({type:'get',url:thisurl,data:_this.data(),success:function(data){
				$('.searchselect3').html(data);
			}});
		}else if($(this).data('keyname')){
			if($(this).data('keyname').indexOf('webshopsort')!=-1&&$(this).hasClass('active')){
				return false;		
			}
			$(this).parent().find('.li').removeClass('select');
			$(this).toggleClass('select');		
			var flag=selobj.hasClass('active');
			$(this).parents('.searchselect3').find('.licont').removeClass('active');
			!flag?selobj.addClass('active'):'';		
			flag?$(this).removeClass('select'):'';
			if($(this).data('keyname').indexOf('webshopsort')!=-1){
	...
	$('.searchselect3').delegate('.tabcont .licont .reset','click',function(){
			$(this).data('ajax',1);
			return searchresult($(this)[0]);	
	});
	$('.searchselect3').delegate('.tabcont .licont div.leval','click',function(){
		/*var _this=$(this).parents('.licont');*/
		$(this).parents('.licont').removeClass('active');
		$(this).parents('.licontlevel').find('.leval').removeClass('active');
		$(this).addClass('active');
		return searchresult($(this)[0]);
	});
});
...
function searchresult(e){
	var _this=$(e);
	var dataobj=_this.parents('.blockinfo').next('.blockinfo');
	if(_this.hasClass('nomore')||_this.hasClass('loading')){
		return false;
	}

	_this.addClass('loading');
	setTimeout(function(){_this.removeClass('loading');},3000);
	_this.data('ajax',1);
	_this.data('loadcss',0);
	
	var url=thisurl,_thisval,_thisval2;
	$('.selecthead .li').each(function(i) {
		if($(this).data('keyname')){
			 _thisval=parseInt($(this).data('val'));
			 _thisval2=parseInt(_this.data($(this).data('keyname')));
			if(typeof(_this.data($(this).data('keyname')))=='undefined'){
				_this.data($(this).data('keyname'),_thisval);
				if(_thisval!=-1){
					if(url.indexOf('?')>-1){
						url+='&'+$(this).data('keyname')+'='+$(this).data('val');
					}else{
						url+='?'+$(this).data('keyname')+'='+$(this).data('val');	
					}
				}
			}else if(_this.data($(this).data('keyname'))&&_thisval2!=-1){
				if(url.indexOf('?')>-1){
					url+='&'+$(this).data('keyname')+'='+_thisval2;
				}else{
					url+='?'+$(this).data('keyname')+'='+_thisval2;	
				}
			}
	...
	_this.is('A')?url=_this.attr('href'):'';
	window.history.pushState({},0,url);
	$.ajax({type:'get',url:thisurl,data:_this.data(),success:function(data){
		var classstr=$('.searchselect3 .flexbox').attr('class');
		var top=$('.searchselect3 .flexbox').css('top');
		$('.searchselect3').html(data);
		$('.searchselect3 .flexbox').attr('class',classstr);
		$('.searchselect3 .flexbox').css('top',top);
		$(window).scrollTop($('.searchselect3').offset().top-$('header').height());
		
		
		var cssarr=$('.searchselect32').find('.pagecss').data('css'); 
		$('.searchselect32').find('.pagecss').remove();
		if(cssarr){
			var pagecssarr=$('.pagecss:first').data('css').split(',');
			cssarr=cssarr.split(',');
			for(var i in cssarr){
		...
		var jsarr=$('.searchselect32').find('.pagejs').data('js');
		$('.searchselect32').find('.pagejs').remove();
		if(jsarr){
			var pagejsarr=$('.pagejs:first').data('js').split(',');	
			jsarr=jsarr.split(',');
			for(var i in jsarr){
