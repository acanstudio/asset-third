...
$(document).ready(function(){
	...
	setTimeout(function(){
		...
		$(window).scroll(function(){
			...
			if(rightlayobj.length>0){
				if(rightlayobj.height()<$('#container .leftlay').height()){
					...
					$('#container .leftlay').height(rightlayobj.height());
				}
			}
		});
		...
		if($("#pagenav .navright").length>0){
			$("#pagenav .navright .icon-fenlei2").hover(function(){
				$(".headblockcont").show();
			});
...
/*/public/js/maigoo/v2019/block/zhaoshang/hotpaihang.js*/
...
function addmorebang(e){
	...
	$.ajax({type:'get',url:window.location.href,data:$(e).data(),success:function(data){
		$('#_ajaxdata').html(data);
		$(e).parent('.item').html($('#_ajaxdata').find('li:first').find('.minbox .item:first').html());
	}});
}

/*/public/js/maigoo/v2019/block/publicblock/article940.js*/
$(document).ready(function(){
...
/*/public/js/maigoo/v2019/block/publicblock/articlecomment.js*/
$(function (){
    var facearr = ['让您久等了','非常抱歉','您先别着急','快点快点','不好意思啦','在吗','非常棒','不明白','再见','无语','微笑','大笑','怒视','惊恐','泪流满面','鼓掌','开心','害羞','点赞','握手','搜索中','比心','安排一下','尴尬','期待','晓得勒','给你点赞','你在吗','暴汗','不明所以'];
    $.fn.smohanfacebox = function(options) {
        ...
        var faceimg = "";
        for (i = 0; i < 123; i++) {
            faceimg += '<li><a href="javascript:void(0)"><img class="lazy" data-src="/public/images/cnpp/smohan/face/' + (i + 1) + '.gif" face="[' + facearr[i] + ']"/></a></li>'
        }
        ...
        btnobj.on(options.Event,
        function(e) {
        	if (typeof(userid)=='undefined' || userid==0){
        		cnpplogin();
        		return;
        	}
			if($("#SmohanFaceBox").find('.lazy').length>0){
				lazyPic();	
			}
            if ($("#SmohanFaceBox").is(":hidden")) {
                ...
                btnobj.removeClass("in")
            }
        });
        facepic.stop().click(function() {
            $("#" + options.textid).stop().inimage($(this).prop("outerHTML"));
        });
    ...
    $.fn.extend({
	    inimage:function(text){
	       	var obj= $(".commentinput")[0];
	       	var range, node;
	       	if(!obj.hasfocus) {
	       		obj.focus();
	       	}
    ...
    $(".commentinput").on('focus',function(){
    	if (typeof(userid)!='undefined' && userid>0){
    		$(".beforeinput").addClass('forinput');
    	}else{
    		$(this).attr('contenteditable',false);
    	}
    })
    $(".beforeinput").click(function(){
    	if (typeof(userid)!='undefined' && userid>0){
    		$(".commentinput").trigger('focus');
    	}
    })

    $(".commentinput,.beforeinput").on('blur',function(){
    	if (!$(this).html()) {
    		$(".beforeinput").removeClass('forinput');
    	}
    })
});
function say(e){
	...
	$(e).attr('load','1');
	var loadobj=setTimeout(function(){$(e).attr('load','0');},5000);
	var contobj=$('#commentinputbox').find('.commentinput');
	if(contobj.html().replace(/\s+/g,'')==''){
		contobj.focus();
		return false;
	}
	var blockid=$('#commentinputbox').find(':hidden[name=blockid]').val();
	var itid=$('#commentinputbox').find(':hidden[name=itid]').val();
	var yingyongid=$('#commentinputbox').find(':hidden[name=yingyongid]').val();

	$.ajax({type:'POST',url:'/ajaxstream/comment/',data:{'action':'say','blockid':blockid,'itid':itid,'yingyongid':yingyongid,'cont':contobj.html()},success:function(data){
		if(data.indexOf('error')!=-1){
			alert(data.split('|')[1]);	
		}else if(data.indexOf('rzphone')!=-1){
			dialog('温馨提醒',data.split('|')[1],function(){window.location.href=CNPP+'user/setphone/'},'立即前往');
		}else{
			
			if($('#commentlist .commentlistbox').length>0){
				$('#commentlist .commentlistbox').after(data);
			}else if($('#pos_commentlist .itembox').length>0){
				$('#pos_commentlist').find('.nodata').remove();
				if ($('#pos_commentlist .itembox .usersay:first').length>0){
					$('#pos_commentlist .itembox .usersay:first').before(data);
				}else{
					$('#pos_commentlist .itembox').after(data);
				}
		...
		$(e).attr('load','0');
		clearTimeout(loadobj);
	}});
	
	return false;
}

/*/public/js/maigoo/v2019/block/comment/commentlist.js*/
...
function reply(obj){
	...
	var contobj=$('#replydialog').find('textarea:first');
	if(contobj.val().replace(/\s+/g,'')==''){
		contobj.focus();
		return false;
	}

	$.ajax({type:'POST',url:'/ajaxstream/comment/',data:{'action':'reply','id':$('#replydialog').find(':hidden[name=id]').val(),'cont':contobj.val()},success:function(data){
		if(data.indexOf('error')!=-1){
			alert(data.split('|')[1]);	
		}else{
			var objparent3 = obj.parent().parent().parent().parent();
			if (obj.parents('dd').length>0){
				obj.parents('dd').append(data);
			}else if(obj.parent().parent().parent().parent().find('.replylist').length>0){

				if (objparent3.find('.replylist dd:first').length>0){
					objparent3.find('.replylist dd:first').before(data);
				}else{
					objparent3.find('.replylist').show();
					objparent3.find('.replylist dl').append(data);
				}
			}
			contobj.val('');
		}
...
function replyDialog(e,id){
	var cont='<div id="replydialog"><div class="replycont"><i class="blue">'+$(e).parents('.nickname').find('.name').html()+'：</i><br/>'+$("#con"+id).html()+'</div>';
	cont += '<div class="replybox"><input type="hidden" name="id" value="'+id+'" /><textarea class="textarea" id="commenttext" placeholder="请输入您要回复的内容"></textarea></div></div>';
	Kdialog($(e),cont,'回复',800);	
}

function nothingComment(){
	if($('#wangyoucontent').length>0&&$('#wangyoucontent .usersay').length<=0){
		$('#wangyoucontent .load_block').html('暂无评论');
		$('#wangyoucontent .blockmorebox').remove();
	}
}
function commentreply(e){
	if (userid<1){
		dialog('温馨提醒','您还未登录，是否立即前往登录?',function(){window.location.href='/user/login/';});
		return;
	}
	nowcommentreply = $(e);
	$('.mobilereplybox').length>0?$('.mobilereplybox').remove():'';
	var commentid = nowcommentreply.attr('commentid');
	var commentuser = nowcommentreply.parent().parent().find('.name').text();
	var commentcontent = $('#con'+commentid).text();
	var replyboxhtml = '<div class="mobilereplybox mobilereplybox_show"><input type="hidden" id="replayforcommentid" value="'+commentid+'"/>';
	replyboxhtml += '<div class="grayLayer"></div><div class="content"><div class="btnBar font16"><div class="fixWidth"><div class="cancel">取消</div><div class="title"><div>评论回复</div></div><div class="replybtn">发表</div></div></div><div class="replaybox">';
	replyboxhtml += '<div class="original"><span class="name">'+commentuser+':</span>'+commentcontent+'</div>';
	replyboxhtml += '<textarea class="textarea font14" id="commenttextreply" placeholder="请输入您要回复的内容"></textarea></div></div></div>';
	$('#container').after(replyboxhtml);
	$(document).keydown(function(event){
	　　  if(event.keyCode == 13 && $('.mobilereplybox_show').length>0){
	　　  		gotoreply(commentid);
	　　   }
	});
	$('.mobilereplybox .cancel').click(function(){
		$('.mobilereplybox').removeClass('mobilereplybox_show');
	})
	$('.mobilereplybox .replybtn').click(function(){
		if($('.mobilereplybox_show').length>0){
			gotoreply(commentid);
		}
	})
	$('.mobilereplybox .grayLayer').click(function(){
		$('.mobilereplybox').removeClass('mobilereplybox_show');
	});
}
function gotoreply(commentid){
	$.ajax({
		url:'/ajaxstream/comment/',
		type:'post',
		data:{'action':'reply','id':commentid,'cont':$('#commenttextreply').val()},
		success:function(data){
			$('#con'+commentid).parent().parent().find('dl dd:last').length>0?$('#con'+commentid).parent().parent().find('dl dd:last').after(data):$('#con'+commentid).parent().parent().find('.replylist dl').html(data).parent().show();
			$('.mobilereplybox').remove();
		}
	})
}