...
/*/public/js/maigoo/v2019/block/publicblock/searchselect.js*/
var thisurl = window.location.pathname;
$(document).ready(function(e) {
	$('.searchselect').delegate('.haveselect dd a','click',function(){//取消条件
		var _this=$(this);
		var _hobj=$('.licont[data-keyname='+_this.data('keyname')+']');
		_this.data('val','');
		_this.find('em').html(_hobj.find('dt em').html());

		var url=getResultData();
...
function searchresult(e){
	...
	$('.haveselect a').each(function(i) {
		if(_this.data($(this).data('keyname'))){
			$(this).data('val',_this.data($(this).data('keyname')));
			$(this).find('em').html(_this.html());	
		}
	});	

	var url=getResultData();
	_this.attr('a')?url=_this.attr('href'):'';
	window.history.pushState({},0,url);
...
function getResultData(){
	var _thisdata={};
	_thisdata.action='searchlist';
	_thisdata.ajax=1;
	_thisdata.loadcss=1;
	
	var url=thisurl,_thisval;
	$('.haveselect a').each(function(i) {
		if($(this).data('keyname')=='catid'&&url.indexOf('list_')>-1&&url.indexOf('goomai')==-1){
			var _thisval=$(this).data('val');
			url=url.replace(/list_(\d+)/g,'list_'+$(this).data('val'));
			_thisdata[$(this).data('keyname')]=_thisval;
		}else{
			var _thisval=$(this).data('val');
			if(_thisval){
				if(url.indexOf('?')>-1){
					url+='&'+$(this).data('keyname')+'='+_thisval;
				}else{
					url+='?'+$(this).data('keyname')+'='+_thisval;	
				}
				_thisdata[$(this).data('keyname')]=_thisval;
	...
	$.ajax({type:'get',url:thisurl,data:_thisdata,success:function(data){
		...
		var cssarr=$('.searchselect').find('.pagecss').data('css'); 
		$('.searchselect').find('.pagecss').remove();
		if(cssarr){
			var pagecssarr=$('.pagecss:first').data('css').split(',');
			cssarr=cssarr.split(',');
			for(var i in cssarr){
		...
		var jsarr=$('.searchselect').find('.pagejs').data('js');
		$('.searchselect').find('.pagejs').remove();
		if(jsarr){
			var pagejsarr=$('.pagejs:first').data('js').split(',');	
			jsarr=jsarr.split(',');
			for(var i in jsarr){
