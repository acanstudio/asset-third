...
/*/public/js/mobile/v2019/block/publicblock/navscroll.js*/
$(document).ready(function(e) {
	/*阻止blockmenu滑动导致页面切换*/
	var startx=0,starty=0,endx=0,endy=0,startpos=0;
	$('#morelist').click(function(e){
		if($(e.target).attr('id')=='morelist'){
			$('#morelist').removeClass('show');
		}
	});
	var topmenuobj = $('.navscroll .navscrollbox');
	if(topmenuobj.find('ul li.active').length>0){
		var menuwidth = topmenuobj.width();
		var liwidth = topmenuobj.find('ul li').first().width();
		var eq = parseInt(topmenuobj.find('ul li.active').index())+2;
		if(liwidth*eq>menuwidth){
			var newwidth = liwidth*eq-menuwidth;
			topmenuobj.animate({scrollLeft:newwidth});
		}
	}
	topmenuobj.bind('touchstart',function(){
		var touch = event.targetTouches[0];
		startx=touch.pageX;
		starty=touch.pageY;
		...
		endx=touch.pageX;
		endy=touch.pageY;
		$(this).scrollLeft(startpos-endx+startx);
		var xpos=endx>startx?endx-startx:startx-endx;
		var ypos=endy>starty?endy-starty:starty-endy;
		if(xpos>ypos){
			event.preventDefault(); //阻止触摸事件的默认行为，即阻止滚屏
...
/*/public/js/mobile/v2019/block/product/specslist2.js*/
$(document).ready(function (){
	$('.productspecslist2 .itembox').find('ul:eq(0)').find('li.item:eq(0)').addClass('up');
	$('.productspecslist2').delegate('li.item .iconbox','click',function(){
		$(this).parents('li.item').toggleClass('up');
	});
});	
