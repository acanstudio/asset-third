...
/*/public/js/mobile/v2019/block/publicblock/searchselect2.js*/
var thisurl = window.location.pathname+'?dynamic=1';
$(document).ready(function(e) {
	$('.searchselect2').delegate('.selecthead .li','click',function(){
		var i=$(this).index(),selobj=$(this).parents('.searchselect2').find('.licont:eq('+i+')');
		var _this=$(this);
		if($(this).data('keyname')){
			$(this).parent().find('.li').removeClass('select');
			$(this).toggleClass('select');		
			var flag=selobj.hasClass('active');
			$(this).parents('.searchselect2').find('.licont').removeClass('active');
			...
			$.ajax({type:'get',url:thisurl,data:_this.data(),success:function(data){
				$('.searchselect2').html(data);
			}});
		}
	});
	$('.searchselect2').delegate('.tabcont .licont .other .btn','click',function(){
			$(this).data('ajax',1);
			return searchresult2($(this)[0]);	
	});
	$('.searchselect2').delegate('.tabcont .licont div.leval.open','click',function(){
			$(this).parents('.licontlevel').addClass('noflag').removeClass('hasflag');
			$(this).hide();
			if(!$(this).siblings('.leval.clicklevel.active')||$(this).parent('.licontlevel').data('key')=='level1'){
				$(this).parents('.licontlevel').siblings('.licontlevel').hide();
			}else{
				$(this).parents('.licontlevel').next('.licontlevel').hide();
			}
			
	});
	$('.searchselect2').delegate('.tabcont .licont div.leval','click',function(){
		if(!$(this).hasClass('open')){			
			var keyname=$('.selecthead').find('.li:eq('+$(this).parents('.licont').index()+')').data('keyname');
			var list=new Array()
			list['catid']='catlist';
			list['areaid']='arealist';
			if(list[keyname]&&!$(this).parents('.licont').hasClass('nolevel')){
				$(this).data('clicktab',list[keyname])
			}
			$(this).parents('.licont').hasClass('nolevel')?$('.searchselect2').find('.selecthead .li.active').click():'';
			searchresult2($(this)[0]);
		}
...
function searchresult2(e){
	...
	$('.searchselect2 .selecthead .li').each(function(i) {
		if($(this).data('keyname')){
			 _thisval=$(this).data('keyname')!='py'?parseInt($(this).data('val')):$(this).data('val');
			 _thisval2=$(this).data('keyname')!='py'?parseInt(_this.data($(this).data('keyname'))):_this.data($(this).data('keyname'));
	...
	if(_this.data('clicktab')){
		url+='&clicktab='+_this.data('clicktab')+'';
	}
	_this.is('A')?url=_this.attr('href'):'';
	window.history.pushState({},0,url);
	$.ajax({type:'get',url:thisurl,data:_this.data(),success:function(data){
		var classstr=$('.searchselect2 .flexbox').attr('class');
		var top=$('.searchselect2 .flexbox').css('top');
		$('.searchselect2').html(data);
		$('.searchselect2 .flexbox').attr('class',classstr);
		$('.searchselect2 .flexbox').css('top',top);
		$(window).scrollTop($('.searchselect2').offset().top-$('header').height());
		
		
		var cssarr=$('.searchselect2').find('.pagecss').data('css'); 
		$('.searchselect2').find('.pagecss').remove();
		if(cssarr){
			var pagecssarr=$('.pagecss:first').data('css').split(',');
			cssarr=cssarr.split(',');
			for(var i in cssarr){
		...
		var jsarr=$('.searchselect2').find('.pagejs').data('js');
		$('.searchselect2').find('.pagejs').remove();
		if(jsarr){
			var pagejsarr=$('.pagejs:first').data('js').split(',');	
			jsarr=jsarr.split(',');
			for(var i in jsarr){
