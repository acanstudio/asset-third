...
/*/public/js/mobile/v2019/block/publicblock/searchselect.js*/
var thisurl = window.location.pathname+'?dynamic=1';
$(document).ready(function(e) {
	$('.searchselect').delegate('.selecthead .li','click',function(){
		var i=$(this).index(),selobj=$(this).parents('.searchselect').find('.licont:eq('+i+')');
		var _this=$(this);
		if(_this.data('keyname')=='pricesort'){//价格
			...
			$(this).parents('.searchselect').find('.licont').removeClass('active');
	...
	$('.searchselect').delegate('.tabcont .licont .reset','click',function(){
			$(this).data('ajax',1);
			return searchresult($(this)[0]);	
	});
	$('.searchselect').delegate('.tabcont .licont div.leval','click',function(){
...
function searchresult(e){
	...
	$.ajax({type:'get',url:thisurl,data:_this.data(),success:function(data){
		var classstr=$('.searchselect .flexbox').attr('class');
		var top=$('.searchselect .flexbox').css('top');
		$('.searchselect').html(data);
		$('.searchselect .flexbox').attr('class',classstr);
		$('.searchselect .flexbox').css('top',top);
		$(window).scrollTop($('.searchselect').offset().top-$('header').height());
